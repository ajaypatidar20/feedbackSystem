...
public class LoginManager {
  ...
  public LoginCredit getLogin(LoginCredit lc) throws SQLException
    ...
    public void insertUserCredit(LoginCredit lc){
        int i = db.executeUpdate("insert into logincredit "
                + "values('"+lc.getUserId()+"',"
                + "'"+lc.getPassword()+"',"
                + "'"+lc.getRole()+"',"
                + "'"+lc.getDeptId()+"',"
    ...
    public void deleteUserCredit(LoginCredit lc){
        int i = db.executeUpdate("delete logincredit "
                + "where user_id='"+lc.getUserId()+"' "
                + "and role='"+lc.getRole()+"'");
    }
    
    public void allowUserLogin(LoginCredit lc){
        int i = db.executeUpdate("update logincredit "
                + "set status=1 where "
                + "user_id='"+lc.getUserId()+"'"
                + "and role='"+lc.getRole()+"");
    }
}
